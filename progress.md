...
- **2026-09-11 (v13 build, in progress):** After eight Codex reviews (v10 → 5/10, v11 → 6/10, v12 → 6/10 with 12 findings, all in `.gpt-outputs/`), the reviewer's consistent verdict is that the prose transaction system cannot reach 9/10; started the restructure it asked for: a tested Node helper `.claude/skills/council/scripts/council.mjs` owning every state transition (atomic `council.json`, rendered `LOG.md`, deterministic tally, generated packets, prepared-result apply, dispatch receipts, orphan-job audit via the companion listing with `CODEX_COMPANION_SESSION_ID` unset, fault-injection tests). Libraries written (store, tally, edits, packets, codex); CLI + tests + skill rewrite + Copilot custom agent pending. Harness fact sheets for the reviewer in `.gpt-outputs/claude-code-harness-facts.md` and `copilot-harness-facts.md`. Key Copilot finding: prompt files are deprecated on the Agent Host; Copilot loads `.claude/skills/*/SKILL.md` and `.github/agents/*.agent.md` instead. See next_session.md for the full plan.
