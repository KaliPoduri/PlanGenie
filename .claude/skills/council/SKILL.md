...
| `IN PROGRESS (round N, merged)` | `planning/packets/round-N-merge.md` written with tallies; nothing applied yet | apply the agreed edits |
| `IN PROGRESS (round N, applied)` | agreed edits applied, round committed | stop-rule check → next round or final review |
| `FINAL REVIEW (k/m)` | `planning/packets/FINAL.md` written; user verdicts 1..k of m open items logged | present open item k+1 |
...
5. Restore the agent files' previous `model:`/`effort:` values, set `STATUS: CLOSED`, update `planning/status/next_session.md`, append `- <ISO timestamp> council closed — <percentage>, <m> open items` to `planning/status/progress.md`, and commit by pathspec (add `planning/packets/FINAL.md` to the list).
...
4. **Continue at the recorded stage** per the STATUS grammar table. Stage-specific rules:
   ...
   - `applied`: check `git log -- planning/council_state/LOG.md` for the round's commit; commit if missing; then the stop-rule check.
   - `FINAL REVIEW (k/m)`: read `planning/packets/FINAL.md`; present open item k+1 onward; verdicts 1..k are in LOG.md — never re-ask them. After the last verdict apply ALL chosen resolutions (including those logged before the pause) in one pass.
